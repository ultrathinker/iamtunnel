...
// no arguments on Windows. It assembles exactly the same building blocks
// the "enrol", "client", "server" and "admin" verbs already use —
// internal/config, internal/client, dialAdmin, sendControl — and hands
// them to internal/ui as plain function values (ui.Actions), because
// internal/ui itself must stay free of role logic (SPEC §7.1) and cannot
// import them back. This file is the one place that wiring happens;
// internal/ui only decides WHEN a value is called and how its outcome is
// worded. None of the wire protocols are duplicated here — guiEnrol calls
// enrolMachine, guiGrant/guiRevoke call dialAdmin, guiServerStop calls
// sendControl, guiServerStart re-execs the same "server start" the CLI
// runs standalone — the same functions the console verbs call, so the
// two paths cannot drift apart at the next protocol change.
//
// The actions themselves live in gui_actions.go, shared with the Linux
// window (IAMT-252); this file keeps only what genuinely is Windows:
// the console detaching and the message box, the CREATE_NO_WINDOW spawn
// attributes and Connect's cmd.exe terminal.
...
// current (IAMT-182) while the window's own actions (Enrol,
// SaveConnection, Machines, ServerStart/Stop, AdminGrant/Revoke) update
// the rest as they complete.
//
// Each of those halves touches only its own. The poll sends a
// ui.LiveUpdate -- the three facts it actually dialled for -- and the
// actions report their results through their return values, which the
// window folds into its own state. There is no third copy of the state
// here; there was one until 21.09.2026, and internal/ui/live_update.go
// records what it cost.
func runGUI(s *streams) int {
	// The console is hidden only once the configuration has loaded: an error
	// before that is shown in a message box when started by a double click
	// (its console would close with the process), and in the terminal
	// otherwise (IAMT-228).
	...
	if err != nil {
		if consoleIsOwn() {
			showErrorBox("iamtunnel could not start the window: " + err.Error())
		}
		return fail(s, err)
	}
	hideOwnConsole()
	...
	elevated, _ := elevate.IsElevated() // an error here means "assume not elevated" — the banner is a hint, never a refusal
	...
	cfg := ui.FrameConfig{
		...
		Actions: ui.Actions{
			...
			ClientConnect: guiClientConnect,
			...
			RestartAsAdmin: func() error {
				...
				// IAMT-445: a file another account can change must not be
				// handed to UAC - that account would be answering the
				// prompt for whoever presses the button.
				if terr := refuseElevatingTamperableExe(exe); terr != nil {
					return terr
				}
				if rerr := elevate.Relaunch([]string{exe}, false); rerr != nil {
					return rerr
				}
				os.Exit(exitOK)
				return nil
	...
	// success value to branch on. internal/ui's Run doc comment is the
	// contract; staticcheck SA4023 (IAMT-187) is the proof that no path
	// of Run yields a nil error.
	runErr := ui.Run(cfg, updates)
	close(done)
	fmt.Fprintf(s.errs, "iamtunnel: could not start the window: %v\n", runErr)
	showErrorBox(fmt.Sprintf("iamtunnel could not start the window: %v", runErr))
	return exitInternal
}

// defaultSpawnServerStart is guiServerStart's real spawn on Windows: the
// same executable, re-invoked with the same argv a console user would
// type. Never called from a test (see guiServerStart's own doc comment).
// CREATE_NO_WINDOW: the window has already detached its own console
// (hideOwnConsole), so without it the child would open a new black
// console window of its own (IAMT-222).
...
// guiClientConnect is the Client screen's Connect button (IAMT-223): a new
// console window running "iamtunnel client connect <machine>" — the session
// itself is a terminal (SPEC §3.1). cmd /k keeps the window open after the
// session ends, so a refusal stays readable (umtunnel's Connect did the same).
// connectTerminalCommand is the console Connect opens. Both cmd.exe in it
// are Windows' own, by full path (R1-CX F-25): the window may be the one
// "Restart as administrator" started, the first cmd.exe was found through
// %PATH%, and the one "start" opens was looked up by cmd itself - current
// directory first.
func connectTerminalCommand(exe, machine string) (*exec.Cmd, error) {
	shell, err := systemTool("cmd.exe")
	if err != nil {
		return nil, err
	}
	// Through "start": exec gives a child NUL as stdin, and a cmd /k reading
	// NUL exits at once. start opens the new window with its own console
	// input; the outer cmd /c needs no window of its own. Its first quoted
	// argument is the window title, the second the program.
	cmd := exec.Command(shell)
	cmd.SysProcAttr = &syscall.SysProcAttr{
		CmdLine:       `"` + shell + `" /c start "iamtunnel ` + machine + `" "` + shell + `" /k ""` + exe + `" client connect ` + machine + `"`,
		CreationFlags: windows.CREATE_NO_WINDOW,
	}
	return cmd, nil
}

func guiClientConnect(machine string) (string, error) {
	...
	cmd, err := connectTerminalCommand(exe, machine)
	if err != nil {
		return "", err
	}
