// helper_watchdog_parent is the test-side "parent" used by the
// doorwatch test in doorwatch_test.go. It mimics a server role:
// installs a door line, writes a control file with the door-id,
// then sleeps until killed. The test kills it with taskkill /f
// (no graceful exit); the spawned doorwatch subprocess must then
// clean up the line.
//
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: helper_watchdog_parent <keyfile>")
		os.Exit(2)
	}
	...
	// Sleep forever. The test will taskkill /f this process; the
